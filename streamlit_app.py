...
def _running_with_streamlit() -> bool:
    """Return True only when launched by `streamlit run ...`."""
    try:
...
def _print_install_guide(missing: list[str]) -> None:
    print(
...
def _print_run_guide() -> None:
    print(
        "[안내] 이 파일은 Streamlit 앱입니다.\n"
        "직접 `python streamlit_app.py`로 실행하면 ScriptRunContext 경고가 발생할 수 있습니다.\n"
        "아래 명령으로 실행하세요:\n"
        "  streamlit run streamlit_app.py"
    )
